   ...
   for ( ; el != end_el; ++el){
      ...
      for (unsigned int qp=0; qp<n_qp; qp++){
         ...
         for (unsigned int i=0; i != n_sf; ++i){
            if (elem->infinite()){
               // here, D(x)*e^ikx is missing. Probably use FEInterface,
               // but first make it working. So far, use it this way.
...
Number calculate_overlap1(EquationSystems& eq_sys, const std::string sys1, int var1, const std::string sys2, int var2 , IntegralType int_type){
   //run at all processors at once:
   //parallel_object_only(); --> can not be used here.
   ...
   for ( ; el != end_el; ++el){
      ...
      //const unsigned int n_qp = qrule.n_points(); --> fails for infinite elements.
      unsigned int n_qp = fe->n_quadrature_points();
      const unsigned int n_sf = cast_int<unsigned int>(dof_indices.size());
