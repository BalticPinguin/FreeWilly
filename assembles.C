     ...
     for (unsigned int qp=0; qp<max_qp; qp++){
        ...
        for (unsigned int i=0; i<n_sf; i++){
          for (unsigned int j=0; j<n_sf; j++){
            ...
            //  phi -> phi*weight*phase ...
            //  DO NOT TRUST THESE RELATIONS. I DON'T AS WELL.
            Se(i,j) += JxW[qp]*((1-dphase[qp]*dphase[qp])*weight[qp]*phi[i][qp]*phi[j][qp]);
            H(i,j)  += JxW[qp]*(co0_5*( (weight[qp]*dphi[i][qp] + dweight[qp]*phi[i][qp])*dphi[j][qp])
               +(pot- E)*Se(i,j)
            );
          }
        }
