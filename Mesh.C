...
void add_sphere_convex_hull_to_mesh(MeshBase& mesh, libMesh::Real radius, unsigned int points_on_sphere, std::vector<Node> geometry);
void add_sphere_convex_hull_to_mesh2(MeshBase& mesh, libMesh::Real r_max, unsigned int points_on_sphere, std::vector<Node> geometry, std::string creator, const double L, const double N);
...
void tetrahedralise_sphere(UnstructuredMesh& mesh, std::vector<Node> geometry, std::string creator, Real r, int NrBall, Real VolConst, Real L, int N){
   ...
   if (creator=="own")
      ...
      add_sphere_convex_hull_to_mesh2(mesh, r, NrBall, geometry, creator, L, N);
...
void add_sphere_convex_hull_to_mesh2(MeshBase& mesh, libMesh::Real r_max, unsigned int points_on_sphere, std::vector<Node> geometry, std::string creator, const double L, const double N){
   ...
   // play with the following parameters:
   //const double L=L ; // gives curvature: the larger L, the more straight line is obtained.
   //const int N= N;
   double x, scale;
